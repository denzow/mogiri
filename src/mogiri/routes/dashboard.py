...
def index():
    ...
    scheduled_jobs = Job.query.filter(Job.schedule_type != "none", Job.is_enabled == True).count()
    total_workflows = Workflow.query.count()
    scheduled_workflows = Workflow.query.filter(Workflow.schedule_type != "none", Workflow.is_enabled == True).count()
    recent_executions = (
