...
> **Warning**: `--host 0.0.0.0` はすべてのネットワークインターフェースにバインドします。mogiri はシェルコマンドを実行できるため、信頼できないネットワークでは使用しないでください。詳細は [SECURITY.md](SECURITY.md) を参照してください。
...
auth:
  ...
  password: ""          # Web UI パスワード (--host 0.0.0.0 には必須)
```
...
| `MOGIRI_PASSWORD` | Web UI パスワード (config.yaml より優先) |
| `MOGIRI_SECRET_KEY` | Flask セッション署名キー |
...
> **Warning**: systemd サービスで `--host 0.0.0.0` を設定する場合、サーバーがすべてのネットワークインターフェースに公開されます。ファイアウォールや VPN 等のネットワーク制御を適切に設定してください。詳細は [SECURITY.md](SECURITY.md) を参照。
